...
- **Ink ramp** — `ink` → `ink2` → `ink3` → `ink4`, calibrated so **every tier clears 5.8:1 against the darkest paper tone**, including the mono captions. Any new colour must clear it too. 4.5:1 is the legibility threshold, not the comfort one — the floor was raised from 4.70:1 because `ink4` carries most of the small type and the reserve was doing nothing.
- **Metadata is not the lever for reading comfort.** A pass that enlarged every caption below 12px made the toolbars and the navigation crowded and did not help anyone read a story; it was reverted. The sizes the surface was designed around are the sizes it keeps. When something reads as tiring, look first at the reading layer — ink4's contrast was the real fault, and it is fixed there.
- **The list description is the one thing that carries size.** A story's `s.dek` is set at 14.5px in the compact and standard rows and 16px in the feature row. It is the text a reader actually uses to decide whether to open something, so it is the text that earns a raise; a caption is not.
- **Type** — two voices and one number. **Newsreader** carries the whole editorial voice: the logotype (`600`), section titles (`500`), and every headline and body (`400`), using its `opsz` axis to draw small and large text differently. **IBM Plex Mono** is for anything that is metadata: navigation, kickers, timestamps, counts, captions. Metadata is uppercase and widely tracked; reading text never is. **Figtree Figures** is the edition date numeral and nothing else, registered as `--font-figures` / `font-figures`.
- **The date numeral is the one permitted third face, and it is fenced.** The Figtree file is subset to `U+0030-0039` (2.7KB) with `unicode-range` declaring it, so it cannot reach text, and it keeps its variable weight axis so the numeral answers `font-medium` rather than rendering at a baked weight. Do not extend it to letters and do not add a fourth face. Note that `.display` is unlayered CSS — it sets `font-family` and would beat a `font-*` utility on the same element, which is why the numeral carries `font-figures` instead of both.
